...
import model.models_3 as models_3
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='C:\\Users\\29703\\Documents\\learn\\program\\DIP\\CounTR-main\\data\\FSC147', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--resume', default='./data/out/pre_4_dir/checkpoint__pretraining_4.pth',
    ...
    parser.add_argument("--wandb", default="dip", type=str)
    parser.add_argument("--team", default="wukuaiqiandexiyiji", type=str)
...
def main(args):
    ...
    # define the model
    model = models_3.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        current_mae = train_mae / (len(data_loader_train) * args.batch_size) * 1.0
        current_rsme = (train_rmse / (len(data_loader_train) * args.batch_size) ** 0.5) * 1.0

        mae_list.append(current_mae)
        rsme_list.append(current_rsme)
    ...
    np.save('val_mae.npy', mae_list)
    np.save('val_rsme.npy', rsme_list)
